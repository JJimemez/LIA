...
float iGain = 0.1; //creating float value and setting to 0.1 
int integralAction; //create value for integral action
unsigned long previousMillis = 0; //create value long
const long interval = 100; //create value as long
unsigned long currentMillis; //create value long
...
pv = analogRead (A0);  //reading value setting it to pv
 ...
 currentMillis =millis(); //read value of current milis
...
if (currentMillis-previousMillis>=interval){
  previousMillis = currentMillis; //making previous milis equal to current milis
  Error = setpoint-pv; //calculating the error
  integralAction = (integralAction+Error); //calculating the integral action
  co = (pGain*Error)+(iGain*integralAction); //calculating the CO
  moreCO= co/10;
...
if (Screen==-1){ //if screen value goes under 0 it will go to 3
    Screen=3;
}
if (Screen==4){ //if screen value goes over 3 it will go to 0
    Screen=0;
}
...
if (Screen==2){
lcd.setCursor(0,0); //is the location of where you will start, (0,0) is the first row begening of the screen
lcd.print("KI="); //this will print the word Ki= in the LCD screen
lcd.print(iGain); //this will print the value of the igain on the LCD screen
delay(30); //a fast delay
lcd.clear(); //clears everything on the LCD screen
if (DownpbState==0){
  iGain=iGain-0.01; //decreasing value of igain by 0.01
  }
 if (UppbState==0){
  iGain=iGain+0.01; //increasing value of igain by 0.01
  }
 }
